...
package top.notcoral.velocity.bandwidth;
...
import top.notcoral.velocity.bandwidth.BvBandwidthStats.PlayerIdentity;

/**
 * Carries packet attribution metadata through the synchronous Netty codec chain.
 */
public final class BvBandwidthContext {

  public static final AttributeKey<InboundSample> INBOUND_SAMPLE =
      AttributeKey.valueOf("bvelocity-bandwidth-inbound");
  public static final AttributeKey<OutboundSample> OUTBOUND_SAMPLE =
      AttributeKey.valueOf("bvelocity-bandwidth-outbound");
  private static final AttributeKey<ArrayDeque<Integer>> INBOUND_WIRE_SIZES =
      AttributeKey.valueOf("bvelocity-bandwidth-inbound-wire-sizes");
  private static final AttributeKey<PlayerIdentity> PLAYER_IDENTITY =
      AttributeKey.valueOf("bvelocity-bandwidth-player-identity");

  private BvBandwidthContext() {
  }
  ...
  public static void recordInbound(ChannelHandlerContext ctx, PacketKey packetKey) {
    ...
    if (sample != null) {
      BvBandwidthStats.INSTANCE.record(packetKey, sample.player(), sample.wireBytes());
    }
  }
