...
import top.notcoral.velocity.bandwidth.BvBandwidthContext.InboundSample;
...
public final class MinecraftBandwidthInboundHandler extends ChannelInboundHandlerAdapter {
  ...
  public void handlerAdded(ChannelHandlerContext ctx) throws Exception {
    BvBandwidthContext.enableInboundWireSizes(ctx.channel());
  }

  @Override
  public void handlerRemoved(ChannelHandlerContext ctx) throws Exception {
    BvBandwidthContext.disableInboundWireSizes(ctx.channel());
  }
  ...
  public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
    ...
    final int observedWireBytes = BvBandwidthContext.pollInboundWireSize(ctx);
    final InboundSample previous = ctx.channel()
        .attr(BvBandwidthContext.INBOUND_SAMPLE)
        .getAndSet(new InboundSample(
            observedWireBytes >= 0
                ? observedWireBytes : ProtocolUtils.varIntBytes(frameBytes) + frameBytes,
            BvBandwidthContext.playerIdentity(ctx.channel(), connection)
        ));
    try {
      ctx.fireChannelRead(msg);
    } finally {
      ctx.channel().attr(BvBandwidthContext.INBOUND_SAMPLE).set(previous);
    }
  }
}
